...
import { motion, useScroll, useTransform, useSpring, useMotionValue, useInView, type MotionValue } from "framer-motion";
...
/* ════════════════════════════════════════════════════════════
   Magnetic card (for the project grid)
════════════════════════════════════════════════════════════ */
function MagneticCard({ project }: { project: (typeof projects)[0] }) {
    ...
    return (
        <motion.div
            ...
            style={{ rotateX: springRX, rotateY: springRY, x: translateX, y: translateY, transformStyle: "preserve-3d" }}
            className="group relative glass-card overflow-hidden cursor-pointer h-full"
        >
            {isHovered && (
                <div className="absolute inset-0 pointer-events-none z-0"
                    style={{ background: `radial-gradient(280px circle at ${spotlight.x}% ${spotlight.y}%, ${project.color}18, transparent 70%)` }} />
            )}
            <motion.div
            ...
            <motion.div
                ...
                style={{ background: `linear-gradient(90deg, transparent, ${project.color}, transparent)`, boxShadow: isHovered ? `0 0 12px ${project.color}60` : "none" }}
            />
            <div className="relative z-10 p-8">
                <div className="flex items-start justify-between mb-6">
                    <motion.div
                        ...
                        className="w-16 h-16 rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-white/5 flex items-center justify-center relative"
                    >
                        {project.image.startsWith("http") ? (
                            <img 
                                ...
                                className={`w-full h-full ${
                                    project.title === "Opus Virtual Offices" 
                                    ? "object-contain p-2" 
                                    : "object-cover"
                                }`} 
                                loading="lazy" 
                    ...
                    <div className="flex gap-3">
                        {[
                            { href: project.github, label: "GitHub", icon: (<svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>) },
                            { href: project.live, label: "Live", icon: (<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>) },
                        ].filter(btn => btn.href !== "#").map(btn => (
                            <motion.a key={btn.label} href={btn.href} target="_blank" rel="noopener noreferrer"
                                aria-label={btn.label} whileHover={{ scale: 1.15, rotate: 5 }} whileTap={{ scale: 0.9 }}
                ...
                <h3 className="text-xl font-bold mb-3 transition-all duration-300 flex items-center gap-3"
                    style={{ backgroundImage: isHovered ? `linear-gradient(135deg, ${project.color}, white)` : undefined, WebkitBackgroundClip: isHovered ? "text" : undefined, WebkitTextFillColor: isHovered ? "transparent" : undefined, color: isHovered ? undefined : "white" }}>
                    {project.title}
                    <span className="text-[10px] uppercase tracking-widest px-2 py-0.5 rounded-md border border-white/10 bg-white/5 font-mono opacity-60" style={{ WebkitTextFillColor: "white" }}>
                        {project.origin}
                    </span>
                </h3>
                <p className="text-sm text-[var(--color-text-muted)] leading-relaxed mb-6">{project.description}</p>
                <div className="flex flex-wrap gap-2">
                    {project.tags.map(tag => (
                        <span key={tag} className="px-3 py-1 text-xs rounded-full border transition-all duration-300"
                            style={{ borderColor: `${project.color}33`, color: project.color, backgroundColor: isHovered ? `${project.color}15` : `${project.color}08` }}>
                            {tag}
                        </span>
                    ))}
                </div>
...
export default function Projects() {
    ...
    const blobY = useTransform(gridProgress, [0, 1], [-80, 80]);

    return (
        <section id="projects" className="relative">
            <div ref={gridRef} className="relative py-24 overflow-hidden">
                <motion.div
                    style={{ y: blobY }}
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10">
                    <ScrollReveal mode="inView" direction="up" className="text-center mb-16">
                        <p className="text-[var(--color-primary)] font-mono text-sm tracking-widest uppercase mb-3">Featured Work</p>
                        <h2 className="text-3xl md:text-4xl font-bold text-white">
                            Things I&apos;ve <span className="text-[var(--color-secondary)]">built</span>
                        </h2>
                    </ScrollReveal>
